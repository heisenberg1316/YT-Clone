...
export const StudioUploader = ({ endpoint, onSuccess } : StudioUploaderProps) => {
    return (
        <div>
            {/* mux default uploader but we have hidden it */}
            <MuxUploader endpoint={endpoint} onSuccess={onSuccess} id={UPLOADER_ID} className="hidden group/uploader" />

            {/* custom uploader design using mux components */}
            <MuxUploaderDrop muxUploader={UPLOADER_ID} className="group/drop mb-10 sm:mb-15 md:mb-0">
                <div slot="heading" className="flex flex-col items-center gap-6">
                    ...
                    <MuxUploaderFileSelect muxUploader={UPLOADER_ID}>
                        <Button type="button" className="rounded-full cursor-pointer bg-white  text-black border border-black hover:bg-black hover:text-white duration-300">
                            Select Files
                        </Button>
