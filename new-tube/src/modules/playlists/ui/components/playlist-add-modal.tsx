...
export const PlaylistAddModal = ({ open, onOpenChange, videoId}: PlaylistAddModalProps) => {
    ...
    return (
        ...
        title="Add to playlsit"
        >
            <div className="flex flex-col gap-2 max-h-75 overflow-y-scroll">
                {isLoading ? (
                ...
                : (
                    playlists?.pages.flatMap((page) => page.items).map((playlist) => (
                        <Button key={playlist.id} variant={"ghost"} className="cursor-pointer w-full justify-start px-2 [&_svg]:size-5" size="lg" disabled={addVideo.isPending || removeVideo.isPending} onClick={() => {
                            ...
                            }}>
                            {
                                playlist.containsVideo ? (
                                    <SquareCheckIcon className="mr-2 text-blue-600" />
                                ) : (
